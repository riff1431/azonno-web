...
export function ReviewListClient({ initialReviews }: ReviewListClientProps) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(isBn ? "Are you sure you want to delete this review?" : "Are you sure you want to delete this review?")) {
      return;
    }
    setDeletingId(id);
  ...
  const columns: Column<any>[] = [
    ...
    {
      key: "rating",
      header: isBn ? "items" : t("column_rating"),
      sortable: true,
      cell: (row: any) => (
    ...
    {
      key: "review",
      header: isBn ? "Reviews  Answer" : t("column_description"),
      cell: (row: any) => (
        <div className="max-w-85 text-xs space-y-1">
          ...
          {row.admin_reply && (
            <span className="text-[10px] text-primary-700 bg-primary-50 px-2 py-0.5 rounded border border-primary-200 font-medium inline-block mt-0.5">
              {isBn ? " Answer:" : "Official Reply:"} &quot;{row.admin_reply}&quot;
            </span>
          )}
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-border pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-text">
            {isBn ? "Customers Reviews Moderation" : t("review_management")}
          </h1>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {[
          { id: "all", label: isBn ? "All Reviews" : "All Reviews", count: reviews.length },
          { id: "approved", label: isBn ? "" : "Approved", count: reviews.filter((r) => r.status === "approved").length },
          { id: "pending", label: isBn ? "" : "Pending", count: reviews.filter((r) => r.status === "pending").length },
          { id: "rejected", label: isBn ? "" : "Rejected", count: reviews.filter((r) => r.status === "rejected").length },
          { id: "spam", label: isBn ? "" : "Spam", count: reviews.filter((r) => r.status === "spam").length },
        ].map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      <DataTable
        ...
        searchKey="comment"
        searchPlaceholder={isBn ? "Reviews   Search..." : t("search_reviews")}
        emptyMessage={isBn ? " Reviews  ।" : t("no_reviews")}
      />
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h3 className="text-base font-bold text-text flex items-center gap-2">
                ...
                {isBn ? "Customers Reviews Answer Enter" : "Reply to Customer Review"}
              </h3>
              <button
            ...
            <div className="rounded-xl bg-surface-secondary p-3.5 text-xs space-y-1">
              ...
              <span className="text-[10px] text-text-muted block mt-1">
                {isBn ? ":" : "Customer:"}{" "}
                {replyModalReview.profiles?.full_name || replyModalReview.profiles?.email || (isBn ? "" : "Customer")}
              </span>
            </div>
            ...
            <form onSubmit={handleSendReply} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-text mb-1">
                  {isBn ? "  Answer" : "Official Store Response"}
                </label>
                <textarea
                  rows={4}
                  required
                  placeholder={
                    isBn
                      ? "your   for !  Productsitems your    We ..."
